...
app.get('/result/:pid', function(req, resp){
  ...
  pg.connect(db_url, function(err, client, done) {
      ...
      var q = 'SELECT (SELECT name FROM Polls WHERE pid = \'' + pid + '\') AS pname, name, count(*) FROM Vote WHERE pid = \'' + pid + '\' GROUP BY name';
      console.log("QUERY = "+q);
      client.query(q, function(err, result) {
      done();
      if (err)
...
app.post('/submit', function(req, resp){
  ...
  else
  {
    pg.connect(db_url, function(err, client, done) {
        if (err)
         { console.error(err); }
        var q = 'INSERT INTO Vote (pid, uid, name) VALUES (\'' + req.body.pid +'\',\''+ req.user + '\',\'' + toTitleCase(req.body.name) + '\')';
        console.log("QUERY = "+q);
        client.query(q, function(err, result) {
        done();
        if (err)
